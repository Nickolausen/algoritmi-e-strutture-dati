...
int ht_insert(HashTable *h, const char *key, int value)
{
    ...
    unsigned long hashed_key = hash_function(h, encode(key));
    HashNode *new_node = hashtable_new_node(key, value, h->items[hashed_key]);
    h->items[hashed_key] = new_node;
...
HashNode *ht_search(HashTable *h, const char *key)
{
    ...
    HashNode *current_node = h->items[hashed_key];
    while (current_node != NULL) 
    {
        if (0 == strcmp(key, current_node->key))
            return current_node;
...
int ht_delete(HashTable *h, const char *key)
{
    ...
    while (current_node != NULL) 
    {
        if (0 == strcmp(current_node->key, key))
        {
            if (prev_node == NULL) /* Eliminazione testa */ 
            {
                HashNode *to_delete = current_node;
                current_node = current_node->next;
                free_node(to_delete);
            }
            else
